...
import {parse} from "nq";

class CollectionListPage extends BaseListPage {
    constructor(props) {
        super(props);
        this.presenter = new CollectionListPresenter(this, findObjectUseCase(), countObjectUseCase(), deleteObjectUseCase(), upsertUseCase());
    }
    ...
    render() {
        ...
        const {objects, selected, count, loading} = this.state;
        if (!schema) return <Progress/>;
        return (<>
            <NavBar
                action={() => (<div className="dropdown dropstart d-inline-block">
                    ...
                    <div className="p-3 p-lg-4">
                        <div className="d-flex justify-content-between align-items-center">
                            ...
                            <div className="text-nowrap">
                                ...
                                <span
                                    className="fs-sm text-nowrap">{selected.length > 0 ? selected.length : objects.length}</span>
                        ...
                        <Table
                            ...
                            selectable
                            collapsable
                            excludeFields={Object.keys(schema.fields)
                                .reduce((acc, key) => {
                            ...
                            className="mt-3"
                            actions={schema._table?.actions.map((action, i) => ({
                                ...action,
                                onClick: this.navigateTo.bind(this, action.navigate + '/' + parse(action.props, {object: objects[i]}))
                            }))}
                        />
                    </div>
                </InfiniteScroll>
